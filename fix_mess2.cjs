...
function fix() {
  ...
  const handlePrintRegex = /const handlePrintReceipt = \(receipt: any\) => \{[\s\S]*?doc\.save\(\`Fine_Receipt_\$\{receipt\.memberName\.replace\(\/\\s\+\/g, '_'\)\}_\$\{Date\.now\(\)\}\.pdf\`\);\s*\};\s*const handleEmailReceipt = \(\) => \{[\s\S]*?window\.location\.href = \`mailto:\$\{receiptData\.memberEmail\}\?subject=\$\{subject\}&body=\$\{body\}\`;\s*\};\s*/g;
  
  code = code.replace(handlePrintRegex, '');
  
  // Now add them properly before the LAST `return (` which is actually the first `return (` of the component
